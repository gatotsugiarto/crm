...
use common\modules\master\models\ProvinceSearch;
...
class ProvinceController extends Controller
{
    ...
    {
        // $model = new Province(['scenario' => 'insertData']);
        $model = new Province();
        ...
        if ($model->load(Yii::$app->request->post()) && $model->validate()) {
            if ($model->save()) {
                $model->getBehavior('tokenProtection')->consumeToken();
                Yii::$app->session->setFlash('success', 'Province created successfully.');
    ...
    public function actionUpdate($id)
    {
        ...
        if ($model->load(Yii::$app->request->post()) && $model->validate()) {
            if ($model->save()) {
                $model->getBehavior('tokenProtection')->consumeToken();
                Yii::$app->session->setFlash('success', 'Province updated successfully.');
                return $this->redirect(['index']);
            }
        }
    ...
    public function actionDelete($id)
    {
        ...
        Yii::$app->session->setFlash('success', 'Province deleted successfully.');
        return $this->redirect(['index']);
    }
    ...
    public function actionReactive($id)
    {
        ...
        // Update Province        $model->status_id = 1;
        $model->save();
        ...
        if (Yii::$app->request->isAjax) {
            ...
            return [
                'success' => true,
                'message' => 'Province activate successfully.',
                'errors' => $model->errors,
            ];
        }

        // fallback non-AJAX
        Yii::$app->session->setFlash('success', 'Province activate successfully.');
        return $this->redirect(['index']);
    }
    ...
    public function actionNonactive($id)
    {
        ...
        // Update Province        $model->status_id = 2;
        $model->save();
        ...
        if (Yii::$app->request->isAjax) {
            ...
            return [
                'success' => true,
                'message' => 'Province non activate successfully.',
                'errors' => $model->errors,
            ];
        }

        // fallback non-AJAX
        Yii::$app->session->setFlash('success', 'Province non activate successfully.');
        return $this->redirect(['index']);
    }
    */
